/*const words = ["LEON", "ELEFANTE", "JIRAFA", "HIPOPOTAMO", "RINOCERONTE", "CEBRA"];
const gridSize = 12;
let grid = [];
let startCell = null;
...
// ✅ FUNCIÓN MODIFICADA CON INTEGRACIÓN DE PROGRESO
function handleMouseUp() {
    if (isPaused) return;
    if (!isMouseDown || currentSelection.length === 0) return;
    ...
    if (found && !foundWords.has(found)) {
        ...
        localStorage.setItem("progresoSopaAnimales", progreso);
